...
A React Native mobile app built with Expo and SQLite that lets users manage their product inventory locally. Add, view, edit, and delete products — complete with name, quantity, price, and optional product images captured using the device’s camera.

## Features

- 📦 Add Products: Create new products with name, quantity, price, and optional image.
- 🖼️ Camera Integration: Capture product images directly from your device camera.
- 🧾 View Products: Display all saved products in a scrollable list.
- ✏️ Edit Products: Update product details like name, quantity, or price.
- 🗑️ Delete Products: Remove products permanently from the local database.
- 💾 Local Storage: Uses SQLite for persistent offline storage — no internet required.
- 🔄 Auto Refresh: Product list updates automatically when you return to the store screen.
